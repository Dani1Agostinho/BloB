...
for (var i = 0; i < array_length(pixels); i++) {
	...
	if point_in_circle(oPlayer.x,oPlayer.y,xstart,ystart,300) {
		_index.xNormal = Approach(_index.xNormal,oPlayer.x,4);
		_index.yNormal = Approach(_index.yNormal,oPlayer.y,4);
	} else {
		_index.xNormal = Approach(_index.xNormal,startX+_index.xSpot-sprite_xoffset,4);
		_index.yNormal = Approach(_index.yNormal,startY+_index.ySpot-sprite_yoffset,4);
	}
